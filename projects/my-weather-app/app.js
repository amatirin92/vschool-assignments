...
app.service('getForecastService', ['$http', function ($http) {
    this.getInfo = function (cityName) {
        var key = '716fed57a85c2acad48488ac3a36e435';
        return $http.get('http://api.openweathermap.org/data/2.5/for?q=' + cityName + '&APPID=' + key + '&units=imperial').then(function (response) {
            return response.data
        })
    }
}])
...
app.controller('mainController', ['$scope', 'getForecastService', function ($scope, getForecastService) {
    var weatherIcons = {
        "200": {
            "label": "thunderstorm with light rain"
            , "icon": "storm-showers"
    ...
    $scope.submit = function (cityName) {
        getForecastService.getInfo(cityName).then(function (response) {
            $scope.lowTemp = Math.round(response.list.main.temp_min) + '°F';
            $scope.description = response.weather[0].description;
